-- Never consume expired Prep stock, even when is_not_fresh has not yet been
-- reconciled by the maintenance scheduler. Keep FIFO ordering deterministic
-- when multiple lots share the same expiry timestamp.
...
-- Keep the denormalized freshness flag in sync for inventory/report screens.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        PERFORM cron.schedule(
            'rims-mark-not-fresh-lots',
            '* * * * *',
            'SELECT public.mark_not_fresh_lots();'
        );
    END IF;
END;
$$;
