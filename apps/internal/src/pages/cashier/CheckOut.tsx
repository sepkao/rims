...
import { ArrowLeft, Banknote, CheckCircle2, CreditCard, Landmark, Printer, ReceiptText, RefreshCw, Sparkles } from 'lucide-react'
import { useNavigate, useSearchParams } from 'react-router-dom'
import { apiFetch } from '../../lib/api'

type PaymentMethod = 'cash' | 'promptpay' | 'card'
type OrderItem = { name: string; quantity: number; status: 'pending' | 'confirmed' | 'cancelled' }
type BillData = {
  session: {
...
const paymentOptions: Array<{ value: PaymentMethod; label: string; note: string; icon: typeof Banknote }> = [
  ...
  { value: 'card', label: 'บัตรเครดิต', note: 'ชำระผ่านเครื่อง EDC', icon: CreditCard },
]
...
export default function CheckOut() {
  ...
  const handleCheckout = async () => {
    ...
    const message = paymentMethod === 'cash'
      ...
      : `ยืนยันว่าตรวจสอบการชำระ${paymentMethod === 'promptpay' ? ' PromptPay' : 'ผ่านเครื่อง EDC'}แล้ว และปิดโต๊ะ?`
      ...
      : <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <aside className="anim-up d-4 overflow-hidden rounded-[26px] border-2 border-[#2D1B17] bg-white shadow-[7px_7px_0_#2D1B17]">
          ...
          <div className="border-t-2 border-[#2D1B17] bg-[#FFFDF9] p-5">
            ...
            {paymentMethod === 'promptpay' && <ExternalPaymentPanel type="promptpay" total={bill.total} reference={paymentReference} onReferenceChange={setPaymentReference} />}
            {paymentMethod === 'card' && <ExternalPaymentPanel type="card" total={bill.total} reference={paymentReference} onReferenceChange={setPaymentReference} />}
            <button disabled={submitDisabled} onClick={() => void handleCheckout()} className="mt-5 w-full rounded-xl border-2 border-[#2D1B17] bg-[#2D1B17] py-3.5 text-sm font-black text-white shadow-[4px_4px_0_#B97861] transition-all hover:-translate-y-0.5 hover:shadow-[6px_6px_0_#B97861] active:translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none">{submitting ? 'กำลังทำรายการ…' : 'ยืนยันการชำระเงินและปิดโต๊ะ'}</button>
          </div>
        </aside>
      </div>}
  </div>
...
function BillSummary({ bill }: { bill: BillData }) {
  ...
  return <section className="anim-up d-3 overflow-hidden rounded-[26px] border-2 border-[#2D1B17] bg-white shadow-[7px_7px_0_#2D1B17]">
    ...
    {bill.items.length > 0 && <div className="border-t-2 border-[#2D1B17] px-6 py-5"><h3 className="text-sm font-black">รายการอาหารสำหรับตรวจสอบ</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{(['pending', 'confirmed', 'cancelled'] as const).map((status) => { const items = bill.items.filter((item) => item.status === status); if (!items.length) return null; return <div key={status} className={`rounded-xl border-2 p-3 ${status === 'pending' ? 'border-amber-700/30 bg-amber-50' : status === 'confirmed' ? 'border-green-800/30 bg-green-50' : 'border-red-700/30 bg-red-50'}`}><p className="text-[10px] font-black uppercase tracking-wider">{status === 'pending' ? 'กำลังทำ / รอเสิร์ฟ' : status === 'confirmed' ? 'ยืนยันแล้ว' : 'ยกเลิก'}</p><ul className="mt-2 space-y-1.5 text-xs font-bold">{items.map((item, index) => <li key={`${item.name}-${index}`} className={`flex justify-between ${status === 'cancelled' ? 'line-through opacity-60' : ''}`}><span>{item.name}</span><span>x{item.quantity}</span></li>)}</ul></div> })}</div></div>}
...
function ExternalPaymentPanel({ type, total, reference, onReferenceChange }: { type: 'promptpay' | 'card'; total: number; reference: string; onReferenceChange: (value: string) => void }) {
  const promptpay = type === 'promptpay'
  const Icon = promptpay ? Landmark : CreditCard
  return <div><div className="rounded-2xl border-2 border-[#2D1B17] bg-[#F1E2CF] p-4 text-center shadow-[3px_3px_0_#2D1B17]"><span className="mx-auto flex h-11 w-11 items-center justify-center rounded-xl border-2 border-[#2D1B17] bg-white"><Icon size={21} /></span><p className="mt-3 text-sm font-black">{promptpay ? 'ให้ลูกค้าสแกนป้าย PromptPay หน้าร้าน' : 'ดำเนินการผ่านเครื่อง EDC'}</p><p className="mt-1 text-xs font-semibold text-[#765F56]">{promptpay ? 'ตรวจสอบชื่อบัญชี ยอดเงิน และสลิปก่อนยืนยัน' : 'ตรวจสอบว่ายอดผ่านแล้วก่อนยืนยัน'}</p><strong className="mt-3 block text-2xl font-black">฿{total.toLocaleString()}</strong></div><label className="mt-4 block text-xs font-black uppercase tracking-wide text-[#73564C]">{promptpay ? 'เลขอ้างอิง / เวลาในสลิป' : 'เลขอ้างอิง EDC'}<input value={reference} onChange={(event) => onReferenceChange(event.target.value)} maxLength={120} className="mt-2 w-full rounded-xl border-2 border-[#2D1B17] bg-white px-4 py-3 text-sm font-bold normal-case outline-none transition focus:-translate-y-0.5 focus:shadow-[4px_4px_0_#B97861]" placeholder={promptpay ? 'เช่น เวลาโอน หรือเลขท้ายรายการ' : 'เลขอ้างอิงจากเครื่อง EDC'} /></label></div>
}

function Receipt({ bill, result, onPrint, onBack }: { bill: BillData; result: CheckoutResult; onPrint: () => void; onBack: () => void }) {
  const methodLabel = result.payment.paymentMethod === 'cash' ? 'เงินสด' : result.payment.paymentMethod === 'promptpay' ? 'PromptPay' : 'บัตรเครดิต'
