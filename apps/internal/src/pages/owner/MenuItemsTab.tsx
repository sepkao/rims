...
export default function MenuItemsTab({ ingredients, onError }: { ingredients: IngredientPreset[]; onError: (message: string) => void }) {
  ...
  const editImagePreview = useMemo(() => editImage ? URL.createObjectURL(editImage) : null, [editImage])
  ...
  const saveEdit = async (item: MenuItem) => {
    ...
    try {
      ...
      if (editImage) {
        ...
        form.append('altText', editName.trim())
        await apiFetch(`/owner/menu-items/${item.id}/image`, { method: 'PUT', body: form })
      }
      setEditingId(null)
      setEditImage(null)
      await loadMenuItems()
    ...
    <div className="grid min-w-0 items-start gap-7 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.35fr)]">
      <form onSubmit={createMenuItem} className="min-w-0 overflow-hidden rounded-[24px] border-2 border-[#2D1B17] bg-white shadow-[7px_7px_0_#2D1B17] xl:sticky xl:top-5">
        ...
        <div className="space-y-5 p-4 sm:p-6">
          ...
          <button type="submit" disabled={creating || !name.trim() || !category || selectedLines.length === 0 || selectedLines.length !== lines.length} className="w-full rounded-xl border-2 border-[#2D1B17] bg-[#2D1B17] px-5 py-3 text-sm font-black text-white shadow-[4px_4px_0_#B97861] disabled:cursor-not-allowed disabled:opacity-40">{creating ? 'กำลังสร้างเมนู…' : 'สร้างและแสดงในหน้าสั่งอาหาร →'}</button>
        </div>
      </form>
      ...
      <section>
        ...
        {loading ? <p className="rounded-2xl border-2 border-dashed border-[#D6D0C4] bg-white p-12 text-center text-sm font-bold text-[#7B726B]">กำลังโหลดเมนู…</p> : <div className="space-y-4">
          {groupedVisibleItems.map((group) => <Fragment key={group.name}>
            ...
            <div className="p-5">{editingId === item.id ? <div className="mb-5 rounded-xl border-2 border-[#B97861] bg-[#FFF8EF] p-4"><div className="grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-black">ชื่อเมนู<input value={editName} onChange={(event) => setEditName(event.target.value)} className="mt-1 w-full rounded-lg border-2 border-[#2D1B17] bg-white px-2 py-2 text-xs" /></label><label className="text-[10px] font-black">หมวดหมู่<select value={editCategory} onChange={(event) => setEditCategory(event.target.value)} className="mt-1 w-full rounded-lg border-2 border-[#2D1B17] bg-white px-2 py-2 text-xs">{categories.map((menuCategory) => <option key={menuCategory.id} value={menuCategory.name}>{menuCategory.name}</option>)}</select></label></div><label className="mt-3 block text-[10px] font-black">คำอธิบาย<textarea value={editDescription} onChange={(event) => setEditDescription(event.target.value)} rows={2} className="mt-1 w-full rounded-lg border-2 border-[#2D1B17] bg-white px-2 py-2 text-xs" /></label><div className="mt-3 rounded-xl border-2 border-dashed border-[#B97861] bg-white p-3"><p className="text-[10px] font-black">รูปเมนู</p><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center"><div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl border-2 border-[#2D1B17] bg-[#F1E2CF]">{editImagePreview || item.imagePath ? <img src={editImagePreview ?? `${API_BASE_URL}${item.imagePath}`} alt={`ตัวอย่างรูป ${editName || item.name}`} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center px-2 text-center text-[9px] font-black text-[#876E65]">ยังไม่มีรูป</div>}</div><label className="min-w-0 flex-1 text-[10px] font-bold text-[#6D5147]">{item.imagePath ? 'เลือกรูปใหม่เพื่อแทนรูปเดิม' : 'เพิ่มรูปให้เมนูนี้'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setEditImage(event.target.files?.[0] ?? null)} className="mt-1.5 block w-full rounded-lg border-2 border-[#2D1B17] bg-[#FFF8EF] px-2 py-1.5 text-[10px] file:mr-2 file:rounded-md file:border-0 file:bg-[#2D1B17] file:px-2 file:py-1 file:text-[10px] file:font-black file:text-white" /><span className="mt-1 block text-[9px] font-semibold text-[#876E65]">JPG, PNG หรือ WebP ไม่เกิน 5 MB{editImage ? ` · เลือกแล้ว: ${editImage.name}` : ''}</span></label></div></div><p className="mt-3 text-[10px] font-black">แก้ไขวัตถุดิบและจำนวนถาด</p><div className="mt-2 space-y-2">{editLines.map((line, index) => <div key={`${line.ingredientId}-${index}`} className="flex items-center gap-2"><select value={line.ingredientId} onChange={(event) => setEditLines((current) => current.map((value, at) => at === index ? { ...value, ingredientId: event.target.value } : value))} className="min-w-0 flex-1 rounded-lg border-2 border-[#2D1B17] bg-white px-2 py-1.5 text-xs">{ingredients.map((ingredient) => <option key={ingredient.id} value={ingredient.id}>{ingredient.name}</option>)}</select><input type="number" min="1" step="1" value={line.quantityRequiredPlates} onChange={(event) => setEditLines((current) => current.map((value, at) => at === index ? { ...value, quantityRequiredPlates: Number(event.target.value) } : value))} className="w-16 rounded-lg border-2 border-[#2D1B17] px-2 py-1.5 text-xs" /><label className="flex items-center gap-1 text-[9px] font-bold"><input type="checkbox" checked={line.removable} onChange={(event) => setEditLines((current) => current.map((value, at) => at === index ? { ...value, removable: event.target.checked } : value))} /> ตัดได้</label>{editLines.length > 1 && <button type="button" onClick={() => setEditLines((current) => current.filter((_, at) => at !== index))} className="text-red-700"><X size={14} /></button>}</div>)}</div><div className="mt-3 flex flex-wrap justify-between gap-2"><button type="button" onClick={() => setEditLines((current) => [...current, { ingredientId: ingredients.find((ingredient) => !current.some((line) => line.ingredientId === ingredient.id))?.id ?? '', quantityRequiredPlates: 1, removable: false }])} className="text-[10px] font-black underline">+ เพิ่มวัตถุดิบ</button><div className="flex gap-2"><button type="button" onClick={() => { setEditingId(null); setEditImage(null) }} className="rounded-lg border-2 border-[#2D1B17] bg-white px-3 py-1.5 text-[10px] font-black">ยกเลิก</button><button type="button" disabled={processingId === item.id} onClick={() => void saveEdit(item)} className="rounded-lg border-2 border-[#2D1B17] bg-[#2D1B17] px-3 py-1.5 text-[10px] font-black text-white disabled:opacity-50">{processingId === item.id ? 'กำลังบันทึก…' : 'บันทึกการแก้ไข'}</button></div></div></div> : <><div className="mb-4 flex items-center justify-between rounded-xl border-2 border-[#2D1B17] bg-[#E8D8CA] px-4 py-3"><div className="flex items-center gap-2"><PackageCheck size={17} /><span className="text-xs font-black">พร้อมขายจาก Prep</span></div><strong className={`text-xl font-black ${item.availableServings < 1 ? 'text-red-700' : 'text-green-800'}`}>{item.availableServings} ชุด</strong></div><div className="grid gap-2 sm:grid-cols-2">{item.ingredients.map((ingredient) => <div key={ingredient.id} className="rounded-xl border border-[#EAE5DF] bg-[#FAF8F5] px-3 py-2.5"><div className="flex justify-between gap-2 text-xs font-black"><span>{ingredient.name}</span><span>{ingredient.quantityRequiredPlates} ถาด/ชุด</span></div><p className="mt-1 text-[10px] font-semibold text-[#7B726B]">Prep มี {Math.floor(ingredient.availablePlates)} ถาด · ทำได้ {Math.floor(ingredient.availablePlates / ingredient.quantityRequiredPlates)} ชุด{ingredient.removable ? ' · ลูกค้าตัดออกได้' : ''}</p></div>)}</div>{item.ingredients.length === 0 && <p className="text-xs font-bold text-red-700">เมนูนี้ยังไม่มี BOM จึงขายไม่ได้</p>}</>} </div>
          </article>)}
          </Fragment>)}
