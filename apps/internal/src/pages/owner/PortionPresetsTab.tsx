...
export default function PortionPresetsTab({ ingredients, loading, setIngredients, onError }: {
  ...
  const selectedDraft = selected ? drafts[selected.id] ?? String(Math.round(selected.defaultPortionSizeKg * 1000)) : ''
  ...
  const saveDraft = async (ingredientId: string, value: string) => {
    const portionSizeGrams = Number(value)
    if (!Number.isSafeInteger(portionSizeGrams) || portionSizeGrams < 1 || portionSizeGrams > 9999) throw new Error('ปริมาณต่อถาดต้องเป็นจำนวนเต็มระหว่าง 1–9,999 กรัม')
    const defaultPortionSizeKg = portionSizeGrams / 1000
...
function PresetDialog({ ingredient, value, saving, onChange, onReset, onSave, onClose }: { ingredient: IngredientPreset; value: string; saving: boolean; onChange: (value: string) => void; onReset: () => void; onSave: () => Promise<void>; onClose: () => void }) {
  ...
  const valid = Number.isSafeInteger(numericValue) && numericValue >= 1 && numericValue <= 9999
  return <div role="presentation" className="fixed inset-0 z-50 flex items-center justify-center bg-[#2D1B17]/70 px-4 py-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section role="dialog" aria-modal="true" aria-labelledby="preset-dialog-title" className="w-full max-w-lg rounded-[26px] border-2 border-[#2D1B17] bg-[#FFFDF9] shadow-[8px_8px_0_#2D1B17]" onMouseDown={(event) => event.stopPropagation()}><header className="flex items-start justify-between gap-4 border-b-2 border-[#2D1B17] bg-[#DBC8B8] px-6 py-5"><div><span className="text-[10px] font-black uppercase tracking-[.16em] text-[#8B5746]">Portion preset</span><h2 id="preset-dialog-title" className="mt-1 text-2xl font-black text-[#2D1B17]">{ingredient.name}</h2><p className="mt-1 text-xs font-bold text-[#75584E]">{ingredient.category === 'meat' ? 'เนื้อสัตว์' : 'ผัก'} · 1 ถาด</p></div><button type="button" aria-label="ปิดหน้าตั้งค่า" onClick={onClose} className="rounded-full border-2 border-[#2D1B17] bg-white px-3 py-1 text-xl font-black leading-none shadow-[2px_2px_0_#2D1B17]">×</button></header><div className="space-y-5 p-6"><label className="block text-xs font-black uppercase tracking-[.06em] text-[#513931]">น้ำหนักต่อถาด (กรัม)<div className="relative"><input autoFocus type="number" min="1" max="9999" step="1" value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border-2 border-[#2D1B17] bg-white px-4 py-3 pr-16 text-lg font-black outline-none focus:shadow-[3px_3px_0_#B97861]" /><span className="absolute right-4 top-1/2 mt-1 -translate-y-1/2 text-sm font-black text-[#80675F]">กรัม</span></div><span className="mt-2 block text-xs font-bold text-[#80675F]">ตัวอย่าง: ต้องการ 350 กรัมต่อถาด ให้กรอก 350</span></label><div className="rounded-2xl border-2 border-[#2D1B17] bg-[#F1E2CF] px-4 py-3 text-xs font-bold leading-5 text-[#60483F]">ระบบจะแปลงหน่วยสำหรับการคำนวณสต็อกให้อัตโนมัติ ค่านี้ไม่ใช่จำนวนถาดที่มีอยู่ในคลัง</div></div><footer className="flex flex-col-reverse gap-3 border-t-2 border-[#2D1B17] bg-[#E7C7B8] px-6 py-5 sm:flex-row sm:justify-end"><button type="button" onClick={onReset} disabled={saving} className="rounded-xl border-2 border-[#2D1B17] bg-white px-5 py-2.5 text-sm font-black disabled:opacity-50">คืนค่าจาก API</button><button type="button" onClick={() => { void onSave() }} disabled={!valid || saving} className="rounded-xl border-2 border-[#2D1B17] bg-[#2D1B17] px-5 py-2.5 text-sm font-black text-white shadow-[4px_4px_0_#B97861] disabled:opacity-50">{saving ? 'กำลังบันทึก…' : 'บันทึก Preset'}</button></footer></section></div>
}
