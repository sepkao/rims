    ...
    <div className="w-full max-w-[1100px]">
      <header className="anim-down d-1 relative mb-7 overflow-hidden rounded-[28px] border-2 border-[#2D1B17] bg-[#E7C7B8] px-7 py-7 shadow-[8px_8px_0_#2D1B17]">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><span className="inline-flex items-center gap-1.5 rounded-full border-2 border-[#2D1B17] bg-white px-3 py-1 text-[10px] font-black shadow-[2px_2px_0_#2D1B17]"><CalendarClock size={13} /> NOTIFICATIONS</span><h1 className="mt-4 text-4xl font-black tracking-[-.035em]">ของต่ำ · ใกล้หมดอายุ</h1><p className="mt-2 text-sm font-bold text-[#6D5147]">สต็อกที่ต่ำกว่าเกณฑ์สั่งซื้อ (เนื้อใน Freezer, ผักใน Prep) และล็อตที่ใกล้ถึงวันหมดอายุ · อัปเดตอัตโนมัติทุก 10 วินาที</p></div>
      ...
      <section className="mb-4">
        <h2 className="mb-3 flex items-center gap-2 text-sm font-black uppercase tracking-wide text-[#6D5147]"><TriangleAlert size={16} /> สต็อกต่ำกว่าเกณฑ์สั่งซื้อ ({lowStockAlerts.length})</h2>
        {!loading && lowStockAlerts.length === 0 ? (
          <div className="rounded-[26px] border-2 border-green-900 bg-green-50 px-7 py-10 text-center shadow-[7px_7px_0_#2D1B17]"><CheckCircle2 className="mx-auto text-green-800" size={38} /><h3 className="mt-3 text-xl font-black text-green-900">สต็อกอยู่ในเกณฑ์ปกติ</h3><p className="mt-1 text-sm font-bold text-green-800">ยังไม่มีวัตถุดิบที่ต่ำกว่าเกณฑ์สั่งซื้อในขณะนี้</p></div>
        ) : loading ? (
          <div className="rounded-[26px] border-2 border-[#2D1B17] bg-white px-6 py-10 text-center text-sm font-bold shadow-[7px_7px_0_#2D1B17]">กำลังตรวจสอบสต็อก…</div>
        ) : (
