...
function CreateUserForm({ onCreated }: { onCreated: () => Promise<void> }) {
  ...
  return (
    <form className="admin-surface mb-6 grid gap-4 rounded-xl bg-white p-5 md:grid-cols-4" onSubmit={async (event) => {
      ...
      <button type="submit" disabled={submitting} className="admin-primary rounded-lg bg-[#4A322F] px-4 py-2 font-bold text-white md:col-start-4">{submitting ? 'Saving…' : 'Create user'}</button>
    </form>
  )
}
