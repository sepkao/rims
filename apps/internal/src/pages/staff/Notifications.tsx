...
export default function Notifications() {
  ...
  return (
    <div className="w-full max-w-[1100px]">
      <header className="system-hero anim-down d-1 relative mb-7 overflow-hidden rounded-[28px] border-2 border-[#2D1B17] bg-[#E7C7B8] px-7 py-7 shadow-[8px_8px_0_#2D1B17]">
            ...
            {alerts.map((alert, index) => {
              ...
              return <article key={alert.ingredientId} className={`anim-up overflow-hidden rounded-[26px] border-2 border-[#2D1B17] shadow-[7px_7px_0_#2D1B17] ${alert.severity === 'critical' ? 'bg-red-50' : 'bg-amber-50'}`} style={{ animationDelay: `${index * 70}ms` }}>
                <div className="flex flex-col gap-5 p-6 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="grid min-w-[330px] grid-cols-3 gap-3"><Metric label="ต้องเติม" value={`${alert.shortagePlates} ถาด`} /><Metric label="น้ำหนัก" value={`${alert.recommendedTransferKg.toFixed(1)} kg`} /><Metric label="Freezer" value={`${alert.freezerAvailableKg.toFixed(1)} kg`} /></div>
                </div>
                <footer className="flex flex-col gap-3 border-t-2 border-[#2D1B17] bg-white/70 px-6 py-4 sm:flex-row sm:items-center sm:justify-between"><p className={`flex items-center gap-2 text-xs font-black ${canFill ? 'text-green-800' : 'text-red-700'}`}><Snowflake size={15} />{canFill ? 'Freezer เพียงพอ' : 'Freezer ไม่พอ กรุณาแจ้ง Owner'}</p><button type="button" onClick={() => navigate(`/staff/transfer-to-thaw-prep?ingredient=${encodeURIComponent(alert.ingredientId)}`)} disabled={alert.freezerAvailableKg <= 0} className="rounded-xl border-2 border-[#2D1B17] bg-[#2D1B17] px-5 py-2.5 text-xs font-black text-white shadow-[3px_3px_0_#B97861] disabled:cursor-not-allowed disabled:opacity-40">แปรรูป</button></footer>
              </article>
            })}
          </div>
        )}
